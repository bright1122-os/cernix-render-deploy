...
<div class="admin-page-head">
    <div><div class="cx-eyebrow">Student Trace Search</div><h1>Student Trace</h1><p>Search by matric number, name, RRR, or token reference and inspect the complete exam access trail.</p></div>
</div>

<section class="admin-section">
    <div class="admin-section-head"><h2>Trace Search</h2><span>Identity, payment, QR, timetable, scans</span></div>
    <div class="admin-section-body">
        <form class="admin-filter" method="GET"><input name="q" value="{{ $queryText }}" placeholder="Matric, name, RRR, token reference"><button class="admin-action">Search</button><a class="admin-action ghost" href="{{ route('admin.student-trace') }}">Reset</a></form>
...
<div class="admin-grid two" style="margin-top:16px">
    ...
    <section class="admin-section">
        ...
        <div class="admin-section-body"><div class="admin-info-list">
            ...
            <div class="admin-info-row"><span class="admin-label">QR</span><span class="admin-value">{{ $token ? $token->status.' · '.Str::limit($token->token_id, 18) : 'No QR token' }}</span></div>
    ...
    <div class="admin-section-body">
        @forelse($scans as $scan)
            <div class="admin-info-row" style="grid-template-columns:auto 1fr auto;align-items:center">
                <span class="admin-status {{ $scan->decision === 'APPROVED' ? 'green' : ($scan->decision === 'DUPLICATE' ? 'amber' : 'red') }}">{{ $scan->decision }}</span>
                <span class="muted">{{ $scan->timestamp }} · {{ $scan->examiner_name ?? 'Examiner unavailable' }} · {{ $scan->ip_address }}</span>
